...
function scanOzonNiche(spreadsheetId) {
  ...
  QUERIES.forEach(function(qp){
    ...
    for(var page=1;page<=pages;page++){
      ...
      var resp;
      try { resp = UrlFetchApp.fetch(url,OPTS); } catch(e){ bad++; continue; }
      if(resp.getResponseCode()!==200){ bad++; continue; }
      var j=null;
      try { j=JSON.parse(resp.getContentText()); } catch(e){ bad++; continue; }
      if(!j || !j.widgetStates){ bad++; continue; }
  ...
  if(total===0){
    throw new Error('Ozon не отдал данные (антибот/челлендж): ' + bad + ' запросов вернули не-JSON. Попробуй позже.');
  }
  return JSON.stringify({ok:true, summary:summary, total:total, data:all});
}

function scanNicheMenu(){
  try {
    var r = scanOzonNiche('1VUf_ryMnXuTkD7PBBfRu36fYScwPQtXDu75Dx_iyJgk');
    var j = JSON.parse(r);
    showAlert('Анализ ниши', 'Готово. Найдено: ' + j.total + '. ' + j.summary);
  } catch(e) {
    showAlert('Анализ ниши — ошибка', String(e && e.message ? e.message : e));
  }
}
