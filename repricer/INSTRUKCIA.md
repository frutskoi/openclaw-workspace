...
## План действий для Босса:

1. Открой таблицу: https://docs.google.com/spreadsheets/d/1KZIFWQ61LUS17LpJTvemSX4jnjZJqB5hjl8E9w9a8fE/edit

2. В верхнем меню нажми: **Extensions → Apps Script**

3. Откроется редактор кода. Удали весь старый код (Ctrl+A → Delete)

4. Скопируй код из файла ниже и вставь в редактор

5. Нажми **Save** (иконка дискеты или Ctrl+S)

6. В поле "Function" выбери: `onOpen`

7. Нажми **Run** (иконка ▶️)

8. Google попросит разрешения — нажми **Review Permissions → Continue → Выбери аккаунт → Allow**

9. Перезагрузи таблицу (F5) — появится меню **Репрайсер**

10. Нажми **Репрайсер → Загрузить данные с ВБ**
...
const API_KEY = '***'; // Замените на API ключ из Настройки
const USER_TOKEN = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...'; // Замените на токен из Настройки
...
Перед запуском нужно заменить токены в коде:
1. `API_KEY` — из листа "Настройки" → "API ключ WB (только чтение)"
2. `USER_TOKEN` — из листа "Настройки" → "Токен авторизованного пользователя WB"

Или напиши мне "добавь чтение токенов из таблицы" — я добавлю эту функцию!