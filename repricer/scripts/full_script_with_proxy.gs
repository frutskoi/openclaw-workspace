// Скрипт загрузки данных с ВБ через прокси
// WB API недоступен напрямую из Apps Script из-за CORS
...
function fetchWBAPI(endpoint, options, useUserToken) {
  var url = PROXY_URL + WB_API_BASE + endpoint;
  ...
  Logger.log('WB API Response: ' + statusCode);
